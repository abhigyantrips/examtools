...
export function assignDuties(
  ...
  const facultyDutyCounts = initializeFacultyDutyCounts(faculty, examStructure.designationDutyCounts);
  ...
  try {
    ...
    const finalValidation = validateFinalAssignments(assignments, facultyDutyCounts);
...
function buildUnavailabilityMap(unavailability: UnavailableFaculty[]): Map<string, Set<string>> {
...
function assignSlotDuties(
  ...
  const availableFaculty = allFaculty.filter(f => !unavailableToday.has(f.facultyId));

  if (availableFaculty.length === 0) {
    errors.push(`Day ${dutySlot.day + 1} Slot ${dutySlot.slot + 1}: No faculty available`);
  ...
  const regularResult = assignRegularDuties(context, facultyDutyCounts, existingAssignments);
  ...
  const bufferResult = assignBufferDuties(context, facultyDutyCounts, existingAssignments);
...
function assignRegularDuties(
  ...
  for (let i = 0; i < context.regularDutiesNeeded; i++) {
    if (availableRooms.length === 0) {
      errors.push(`Day ${context.day + 1} Slot ${context.slot + 1}: No more rooms available for regular duties`);
      break;
    }
    ...
    if (eligibleFaculty.length === 0) {
      errors.push(`Day ${context.day + 1} Slot ${context.slot + 1}: No eligible faculty for regular duty ${i + 1}`);
      break;
    }
    ...
    const facultyDutyCount = facultyDutyCounts.find(f => f.facultyId === selectedFaculty.facultyId)!;
...
function assignBufferDuties(
  ...
  for (let i = 0; i < context.bufferDutiesNeeded; i++) {
    ...
    if (eligibleFaculty.length === 0) {
      warnings.push(`Day ${context.day + 1} Slot ${context.slot + 1}: No eligible faculty for buffer duty ${i + 1}`);
      continue;
    }
...
function hasConsecutiveSlotConflict(
  ...
  const dayAssignments = existingAssignments.filter(a => 
    a.facultyId === facultyId && a.day === context.day
  );
...
function checkConstraintViolations(assignments: Assignment[]): string[] {
  ...
  for (const assignment of assignments) {
    if (assignment.isBuffer) {
      bufferCounts.set(assignment.facultyId, (bufferCounts.get(assignment.facultyId) || 0) + 1);
    }
  }

  for (const [facultyId, count] of bufferCounts) {
    if (count > 1) {
      violations.push(`Faculty ${facultyId} has ${count} buffer duties (max 1 allowed)`);
    }
  }
...
export function getAssignmentStats(
  ...
  const stats: Record<string, { regular: number; buffer: number; total: number }> = {};
